...
public class MachineSyncServices extends AsyncTask<Void, Void, String> {

	private ProgressDialog progressBar;

	private MachineSyncListner callback;
	...
	public MachineSyncServices(Activity activity, String data, Fragment frag) {
		 ...
		 */
		this.callback = (MachineSyncListner) frag;
	}
	...
	protected String doInBackground(Void... params) {
		...
		String response = "";
		try {
			URL url = new URL(Config.base_url + "engservices/eng_sync_site_machine");

			List<NameValuePair> profileData = new ArrayList<NameValuePair>();
			profileData.add(new BasicNameValuePair("sync_machine", data));
	...
	protected void onPostExecute(String result) {
		...
		progressBar.dismiss();
		try {
			callback.getMachineSyncSuccess(result);
		} catch (Exception e) {
			// TODO: handle exception
		}
	}
